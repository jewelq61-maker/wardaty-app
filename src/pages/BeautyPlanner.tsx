...
export default function BeautyPlanner() {
  ...
  const fetchBeautyActions = async () => {
    ...
    setBeautyActions((data || []) as BeautyAction[]);
  };
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {['once', 'daily', 'weekly', 'monthly'].map((freq) => (
                    <button
                      key={freq}
                      onClick={() => setNewAction({ ...newAction, frequency: freq as any })}
                      className={cn(
                        "px-3 py-2.5 rounded-xl text-xs font-medium border-2 transition-all",
                        newAction.frequency === freq
                  ...
                  {['morning', 'afternoon', 'evening', 'night'].map((time) => (
                    <button
                      ...
                      onClick={() => setNewAction({ 
                        ...newAction, 
                        time_of_day: newAction.time_of_day === time ? undefined : time as any 
                      })}
                      className={cn(
                        "px-3 py-2.5 rounded-xl text-xs font-medium border-2 transition-all",
                        newAction.time_of_day === time
                          ? "border-secondary bg-secondary/10 text-secondary"
                          : "border-border bg-background text-muted-foreground hover:border-primary/30"
                      )}
                    >
